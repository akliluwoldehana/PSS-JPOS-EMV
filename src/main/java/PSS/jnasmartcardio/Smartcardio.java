...
import PSS.jnasmartcardio.Winscard.Dword;
import PSS.jnasmartcardio.Winscard.DwordByReference;
import PSS.jnasmartcardio.Winscard.SCardReaderState;
...
public class Smartcardio extends Provider {
	...
	public static void justCheck(){
		...
		System.out.println("I am Smartcardio.");
	}
	...
	public static class JnaCardChannel extends CardChannel {
		...
		private ByteBuffer transmitImpl(byte[] command, ByteBuffer response) throws CardException, JnaPCSCException {
			...
			for (int i=0; i<8; i++) {
				...
				if ((byte)0x6c == sw1) {
					...
					// commandBuffer.put(command[0]);
					// commandBuffer.put(command[1]);
					// commandBuffer.put(command[2]);
					// commandBuffer.put(command[3]);
					// commandBuffer.put(command[4]);
		 ...
		 */
		static byte getClassByte(byte origCla, int channelNumber) { // This is the function causing the problem
			if (((byte)0x80 & origCla) != (byte)0x00) { 
				// Not an interindustry class; don't touch it.
				return origCla;
			}
			int cla;
			// 7816-4/2005 5.1.1 Class byte
			if ((byte)0x00 <= channelNumber && channelNumber <= (byte)0x03) {
				// First interindustry values of CLA
				// Class byte is 000x xxcc, where cc is channel number
				cla = (origCla & (byte)0x1c) | channelNumber;
			} else if ((byte)0x04 <= channelNumber && channelNumber <= (byte)0x13) {
				// Further interindustry values of CLA
				// Class byte is 01xx cccc, where cccc is channel number - 4.
				int channelBits = channelNumber - 4;
				cla = (origCla & (byte)0x30) | channelBits | (byte)0x40;
			} else {
